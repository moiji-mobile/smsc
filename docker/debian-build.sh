...
RUN apt-get install --yes --no-install-recommends build-essential equivs devscripts git-buildpackage libc6-dev g++
...
RUN git add OsmoSmsc.image OsmoSmsc.changes
RUN git config --global user.email "holger@freyther.de"
RUN git config --global user.name "Holger Freyther"
RUN git commit -m "Add pharo files inside the docker build"

CMD ${TRAVIS_DEBIAN_GIT_BUILDPACKAGE} --git-ignore-branch --git-ignore-new --git-export-dir=${TRAVIS_DEBIAN_BUILD_DIR} --git-builder='debuild -i -I -uc -us -sa'
EOF
